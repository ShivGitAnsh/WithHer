    ...
    startItineraryGeneration(async () => {
      try {
        ...
        toast({
          ...
          description: itineraryPlan.fallbackUsed
            ? 'Fallback itinerary was used because Gemini generation is unavailable right now.'
            : 'SARA created a safer day-by-day route for this trip.'
        });
      } catch (error) {
        toast({
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                      {data.safetyScore.fallbackUsed
                        ? `${data.safetyScore.status} · fallback score`
                        : `${data.safetyScore.status} · Gemini generated`}
                    </p>
                  </div>
                ...
                <>
                  ...
                  <div className="mt-5 rounded-[1.35rem] border border-border/70 bg-slate-50/90 px-5 py-5">
                    <div className="flex flex-wrap gap-3">
                      ...
                      {data.itineraryPlan.fallbackUsed ? (
                        <Badge variant="caution">Fallback itinerary</Badge>
                      ) : (
                        <Badge variant="success">Gemini itinerary</Badge>
                      )}
                    </div>
